 ...
 */
export function mapMeshyStatus(status: MeshyStatus): ProviderTaskStatus {
  switch (status) {
    case 'PENDING':
      return 'pending';
    case 'IN_PROGRESS':
 ...
 */
export function mapMeshyTaskStatus(task: MeshyTaskResponse): TaskStatusResult {
  return {
 ...
 */
export function extractMeshyDownloads(task: MeshyTaskResponse): DownloadResult {
  const files: DownloadableFile[] = [];

  if (task.model_urls) {
    const urls = task.model_urls;
    if (urls.glb) files.push({ url: urls.glb, name: 'model.glb', format: 'glb' });
    if (urls.fbx) files.push({ url: urls.fbx, name: 'model.fbx', format: 'fbx' });
    if (urls.obj) files.push({ url: urls.obj, name: 'model.obj', format: 'obj' });
    if (urls.usdz) files.push({ url: urls.usdz, name: 'model.usdz', format: 'usdz' });
  }
