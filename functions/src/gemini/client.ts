...
export class GeminiClient {
      ...
      try {
        ...
        if (i > 0) {
          await new Promise((resolve) =>
            setTimeout(resolve, MIN_DELAY_BETWEEN_CALLS_MS)
          );
        }
  ...
  private async generateSingleView(
    ...
    if (analysis.blockReason) {
      throw new Error(
        `Content blocked by Gemini for ${angle} view: ${analysis.blockReason}. ` +
    ...
    if (analysis.safetyIssues.length > 0 && !analysis.hasImage) {
      throw new Error(
        `Image generation blocked for ${angle} view due to safety concerns: ${analysis.safetyIssues.join(', ')}`
      );
    }
    ...
    if (!analysis.hasImage) {
      ...
      throw new Error(
        `No image data in Gemini response for ${angle} view.` +
