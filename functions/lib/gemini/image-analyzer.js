...
function mapStructuredAnalysisResponse(raw, expectedColorCount, locale) {
    const colorPalette = [...new Set(raw.colors
            .map((color) => color.trim().toUpperCase())
            .filter((color) => /^#[0-9A-F]{6}$/.test(color)))].slice(0, expectedColorCount);
    ...
    if (raw.limbs.trim() && raw.limbs.trim().toLowerCase() !== 'none')
        keyFeatures.limbs = raw.limbs.trim();
    if (raw.accessories.length)
        keyFeatures.accessories = cleanStringList(raw.accessories);
    if (raw.distinctiveMarks.length)
        keyFeatures.distinctiveMarks = cleanStringList(raw.distinctiveMarks);
    if (raw.asymmetricFeatures.length)
        keyFeatures.asymmetricFeatures = cleanStringList(raw.asymmetricFeatures);
    if (raw.surfaceTextures.length)
        keyFeatures.surfaceTextures = cleanStringList(raw.surfaceTextures);
...
function parseLegacyAnalysisResponse(text, expectedColorCount) {
...
function parseAnalysisResponse(text, expectedColorCount, locale) {
    const trimmed = text.trimStart();
    if (trimmed.startsWith('{') || trimmed.startsWith('```')) {
        const structured = (0, image_analysis_schema_1.parseStructuredImageAnalysis)(text);
    ...
    try {
        const response = await axios_1.default.post(`${GEMINI_API_BASE}/${MODEL}:generateContent`, {
            ...
            generationConfig: {
                ...
                responseSchema: image_analysis_schema_1.IMAGE_ANALYSIS_RESPONSE_SCHEMA,
            },
        }, {
            headers: {
