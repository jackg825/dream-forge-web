...
exports.pollGeminiBatchJobs = exports.submitGeminiBatch = void 0;
 ...
 */
exports.pollGeminiBatchJobs = functions
    .region('asia-east1')
    .runWith({
    timeoutSeconds: 300,
    memory: '1GB',
    ...
    for (const jobDoc of jobsSnapshot.docs) {
        ...
        try {
            ...
            const newStatus = batch_client_1.GeminiBatchClient.mapJobState(status.metadata.state);
            // Update job status
            const updateData = {
 ...
 */
async function processCompletedBatchJob(jobRef, job, statusResponse, client) {
    functions.logger.info('Processing completed batch job', {
        batchJobId: jobRef.id,
        pipelineId: job.pipelineId,
    });
    // Parse results
    const results = client.parseResults(statusResponse, job.requests);
        ...
        if (result.success && result.imageBase64) {
            try {
                ...
                if (result.viewType === 'mesh') {
                    meshImages[result.angle] = processedImage;
                }
                else {
                    textureImages[result.angle] = processedImage;
                }
 ...
 */
async function handleFailedBatchJob(jobRef, job, errorMessage) {
