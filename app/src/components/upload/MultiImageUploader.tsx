  ...
  const renderDropzone = (angle: ViewAngle, size: 'large' | 'small' = 'large') => {
      ...
      return (
        <div className={`relative rounded-lg overflow-hidden border-2 border-green-500 bg-gray-50 ${heightClass}`}>
          ...
          <button
            ...
            aria-label={t('uploadModes.removeImage')}
          >
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-gray-600">{angleLabel}</p>
        {size === 'large' && (
          <p className="text-xs text-gray-400 mt-1">{t('uploadModes.clickOrDrag')}</p>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 p-1 bg-gray-100 rounded-lg">
        {(Object.entries(INPUT_MODE_CONFIG) as [InputMode, typeof INPUT_MODE_CONFIG[InputMode]][]).map(
          ([key, config]) => (
            <button
              key={key}
              type="button"
              ...
              `}
            >
              <span>{t(`uploadModes.${config.labelKey}.label`)}</span>
              <span className="ml-1 text-xs opacity-70">{t('uploadModes.credits', { count: config.credit })}</span>
            </button>
          )
        )}
      </div>
      ...
      {/* Mode description */}
      <p className="text-sm text-gray-500">{t(`uploadModes.${INPUT_MODE_CONFIG[mode].labelKey}.description`)}</p>
              ...
              <div className="flex flex-wrap gap-2">
                {(['back', 'left', 'right', 'top'] as ViewAngle[]).map((angle) => (
                  <button
                    ...
                    `}
                  >
                    {t(`pipeline.angles.${angle}`)}
                  </button>
                ))}
              </div>
              <p className="mt-2 text-xs text-gray-500">
                {t('uploadModes.selectedAngles', { count: selectedAngles.length })}
              </p>
            </div>
          )}
        </div>
      ...
      {images.size > 0 && (
        <div className="flex items-center justify-between text-sm text-gray-600 pt-2 border-t">
          <span>
            {t('uploadModes.imagesUploaded', { count: images.size })}
            {mode === 'ai-generated' && selectedAngles.length > 0 && (
              <span className="text-indigo-600"> {t('uploadModes.aiGenerating', { count: selectedAngles.length })}</span>
            )}
          </span>
          <span className="font-medium">
            {t('uploadModes.creditsConsumed', { count: CREDIT_COSTS[mode] })}
          </span>
        </div>
      )}
    </div>
